...
def gen_hash(params):
    ...
    hashed_payload = "".join([str(params[key])
                              for key in keys]) + settings.RAVEPAY_SECRET_KEY
    return sign_request(hashed_payload)

def sorting(email="", first_name="", last_name="", description="",
            amount=200, phone="", country="", currency="", ref=""):
    params = {
...
def ravepay_button(
    context, button_id="django-rave-button",button_class="",phone=None,country="NG",currency="NGN",
    email=None,first_name=None,last_name=None,description=None,amount=None,ref=None,redirect_url=None):
    new_ref = ref
    ...
    params = sorting(email=email,first_name=first_name,last_name=last_name,
                     description=description,
                     amount=new_amount,phone=phone,country=country,currency=currency,
                     ref=new_ref)
    return {"data":json.dumps(params),
    "button_id":button_id,
