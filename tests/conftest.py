...
def ravepay_api():
    return RavepayAPI(
        public_key="public_key", secret_key="secret_key", django=False, test=True
    )
...
@pytest.fixture
def mock_assertion(headers, ravepay_api: utils.RavepayAPI):
    def _mock_assertion(mock_call, path, **kwargs):
        side_effect = kwargs.pop("side_effect", None)
        url = "{}{}".format(ravepay_api.base_url, path)
        if side_effect:
