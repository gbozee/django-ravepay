...
def test_successful_order_verification(client, mocker):
    module = mocker.patch("ravepay.utils.RavepayAPI")
    ...
    response = client_instance.get(
        "/verify-payment/1234", params={"amount": "30.23", "code": "biola23"}
    )
    assert response.status_code == 200
    mock_instance.verify_payment.assert_called_once_with(
        "biola23", amount="30.23", amount_only=True
    )
    assert response.json() == {"status": True, "order": "1234"}


def test_generate_account_number(client, mocker):
    module = mocker.patch("ravepay.utils.RavepayAPI")
    mock_instance = module.return_value
    mock_instance.transaction_api.create_payment_account.return_value = (
        True,
        "Successful",
        {"bankname": "Test Bank"},
    )
    ...
    mock_instance.transaction_api.create_payment_account.assert_called_once_with(
        "Eleja", "john@example.com", is_permanent=True
    )
...
def test_webhook_notification(client, mocker):
    ...
    response = client_instance.post(
        "/webhook", json=body, headers={"verif-hash": "encoded signature"}
    )
    assert response.status_code == 200
